...
<header id="header" class="header fixed-top d-flex align-items-center">
  ...
  <main id="main" class="main">
    ...
    <div class="card">
    <div class="card-body">
      <h5 class="card-title">Medicine Update Form</h5>

      <!-- Vertical Form -->
      <form class="row g-3" method="post">
        <?php
        $id = $_GET['id'];  // Assuming you're passing the MedicineID in the URL as a query parameter
        $con = mysqli_connect('localhost', 'root', '', 'Faaruuq');  // Database connection
        
        // Fetch the specific medicine by ID
        $sql1 = "SELECT * FROM medicines WHERE MedicineID = $id";
        ...
        <div class="col-6">
          ...
          <input type="text" class="form-control" id="medicineName" name="name" value="<?php echo $row['Name']; ?>">
        </div>
        
        <div class="col-6">
          <label for="medicinePrice" class="form-label">Price</label>
          <input type="number" step="0.01" class="form-control" id="medicinePrice" name="price" value="<?php echo $row['Price']; ?>">
        </div>
        
        <div class="col-6">
          <label for="createdAt" class="form-label">Created At</label>
          <input type="datetime-local" class="form-control" id="createdAt" name="created_at" value="<?php echo date('Y-m-d\TH:i', strtotime($row['CreatedAt'])); ?>">
        </div>

        <div class="col-6">
          <label for="updatedAt" class="form-label">Updated At</label>
          <input type="datetime-local" class="form-control" id="updatedAt" name="updated_at" value="<?php echo date('Y-m-d\TH:i', strtotime($row['UpdatedAt'])); ?>">
        </div>
      ...
      if (isset($_POST['update'])) {
          ...
          // Update query
          $sql2 = "UPDATE medicines SET Name = '$medicineName', Price = '$price', CreatedAt = '$createdAt', UpdatedAt = '$updatedAt' WHERE MedicineID = $id";
          $result = mysqli_query($con, $sql2);

          if ($result) {
              echo "<div class='alert alert-success'>Medicine updated successfully!</div>";
          } else {
              echo "<div class='alert alert-danger'>Error updating medicine. Please try again.</div>";
          }
      }
      ?>
    </div>
