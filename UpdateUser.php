        ...
        <div class="card-body">
            ...
            <form class="row g-3" method="post" enctype="multipart/form-data">
                ...
                <div class="col-6">
                    ...
                    <img src="uploads/<?php echo htmlspecialchars($row['ProfileImage']); ?>" alt="User Profile" width="100" height="100"><br><br>
                    <input type="file" class="form-control" id="profileImage" name="profileImage">
                    <small class="form-text text-muted">Leave blank if you don't want to change the profile image.</small>
                </div>
                ...
                if (!empty($username) && !empty($role)) {
                    ...
                    if (!empty($profileImage)) {
                        if (move_uploaded_file($profileImageTmpName, $uploadDirectory)) {
                            ...
                            echo "<div class='alert alert-danger'>Failed to upload image. Please check folder permissions and ensure the folder exists.</div>";
                        }
                    } else {
                        $profileImage = $row['ProfileImage']; // Keep the old image if no new image is uploaded
                    }
                    ...
                    if (!empty($password)) {
                        ...
                        $sql2 = "UPDATE users SET Username = ?, Role = ?, PasswordHash = ?, ProfileImage = ? WHERE UserID = ?";
                        $stmt = $con->prepare($sql2);
                        $stmt->bind_param("ssssi", $username, $role, $hashedPassword, $profileImage, $id);
                    } else {
                        // Update without changing password
                        $sql2 = "UPDATE users SET Username = ?, Role = ?, ProfileImage = ? WHERE UserID = ?";
                        $stmt = $con->prepare($sql2);
                        $stmt->bind_param("sssi", $username, $role, $profileImage, $id);
                    }
