        ...
        <div class="card-body">
          ...
          <table class="table datatable">
            ...
            <tbody>
              ...
              // Query to fetch doctors from the Doctors table
              $sql = "SELECT * FROM Doctors";

              if ($result = mysqli_query($con, $sql)) {
                if (mysqli_num_rows($result) > 0) {
                  // Loop through each row in the Doctors table
                  while ($row = mysqli_fetch_array($result)) {
                    ?>
                    <tr>
                      <td><?php echo htmlspecialchars($row['DoctorID']); ?></td>
                      <td><?php echo htmlspecialchars($row['DoctorName']); ?></td>
                      <td><?php echo htmlspecialchars($row['DoctorNumber']); ?></td>
                      <td><?php echo htmlspecialchars($row['Service']); ?></td>
                      <td>
                        <a href="updateDoctor.php?id=<?php echo $row['DoctorID']; ?>"><button type="button" class="btn btn-success">Update</button></a>
                        <a href="deleteDoctor.php?id=<?php echo $row['DoctorID']; ?>"><button type="button" class="btn btn-danger">Delete</button></a>
                      </td>
                    </tr>
                    <?php
                  }
