    ...
    <ul class="sidebar-nav" id="sidebar-nav">
      ...
      <li class="nav-item">
        ...
        <ul id="reports-nav" class="nav-content collapse show" data-bs-parent="#sidebar-nav">
          ...
          <li>
            <a href="MedicinesReports.php" class="active">
...
<main id="main" class="main">
  ...
  <div class="pagetitle">
    ...
    <nav>
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Medicines Report</li>
      </ol>
    </nav>
  </div><!-- End Page Title -->
  ...
  <div class="card">
    <div class="card-body">
      <h5 class="card-title">Medicines Report Form</h5>

      <!-- Vertical Form -->
      <form class="row g-3" method="POST">
        <div class="col-4">
          <label for="inputFromDate" class="form-label">From Date</label>
          <input type="date" class="form-control" id="inputFromDate" name="from_date" required>
        </div>
        <div class="col-4">
          <label for="inputToDate" class="form-label">To Date</label>
          <input type="date" class="form-control" id="inputToDate" name="to_date" required>
        </div>
        <div class="text-left">
      ...
      if (isset($_POST['search'])) {
        ...
        // Query to fetch medicines created within the specified date range
        $query = "SELECT * FROM Medicines WHERE CreatedAt >= '$from_date' AND CreatedAt <= '$to_date'";
        $result = mysqli_query($con, $query);

        // Query to count total medicines created within the specified date range
        $result2 = mysqli_query($con, "SELECT COUNT(*) AS TotalMedicines FROM Medicines WHERE CreatedAt >= '$from_date' AND CreatedAt <= '$to_date'"); 
      ?>
      <table class="table datatable">
        ...
        <tbody>
          ...
          if ($result && mysqli_num_rows($result) > 0) {
            ...
            echo "<tr><td colspan='5' style='text-align:center;'>No records found for the selected date range.</td></tr>";
          }
          ?>
        </tbody>
        <tfoot>
          <tr>
            <td colspan="4" style="text-align: left; font-weight: bold; font-size: 18px;">Total Medicines:</td>
            <td style="text-align: left; font-weight: bold; font-size: 18px;">
              <?php 
              // Fetch the total count of medicines
              if ($r = mysqli_fetch_array($result2)) { 
                echo $r['TotalMedicines']; 
              } else {
                echo 0; // Default if no total is returned
              } 
              ?>
            </td>
          </tr>
